...
void CGLRenderer::SendMoreData()
{
	...
	currentXPos += X_STEP;
	void *content = glMapBufferRange(GL_ARRAY_BUFFER, 6 * 6 * sizeof(GLfloat) * m_dataOffset, 6 * 6 * sizeof(GLfloat), GL_MAP_WRITE_BIT);
	memcpy(content, vertices, 6 * 6 * sizeof(GLfloat));
